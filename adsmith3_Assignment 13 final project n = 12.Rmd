...
This notebook contains the R-script code for the final project. Specifically, the purpose of this assignment is to explore group selelction formulations and solutions. I original wanted to set it up using the traditional LP model via lpsolveAPI, but decided against it. The traditional factors of sucess (i.e. talent, emotional stability, expertise, gender, etc.) are rarely predictive in group perferences and determining successfully ineraction and performance. I decided to let each individual use the three factors that I will outline in the following problem formulation, decide on their own weights for each factor, and evaluate the individuals that are chairing each project group and generate a rating perference. The members will chose their first group perference based based on thir highest score, followed by their second, and third choices.
...
Then each student/individual would inspect each available group that is available based on type of project, minimu levels of gender diversity, and required levels of experience. It is assumed that these minimun qualifications and descriptions are known to our stduents/individuals, or preferences cannot be used in the selection process. Then each one would calculate the expected value of each group based on their rating and weight selection. This program then assumes that these rankings have been made and ready for group placement based on their group project placement. Having spent years in industry and academic environments, I understand teh complexities and subjectivity in placing students/individuals into projects. There are too many interactions that making a simple linear programming model or regission equation cannot capture. I believe that allowing for studnets/individuals to subjective rank their preferences based on their own value of these 3 factors may be the best way to proceed.
...
Of course, with only 12 indidivuals and three projects, it was almost certain that we could accommodate them. The challenge would be with a significantly larger n-size. That can be simply done by changing the inital settings at the beginning of the r-script.
